...
import numpy as np
import math  
from .hsi_utils import makedir, flatten_hsi, flatten_hsis, get_savedir, simple_plot, show_image
...
def decompose(hsiList, method = 'pca', n_components=10):    
    ...
    if method == 'pca': 
        ...
        numDecom = hsiList[0].shape[2] - n_components
        decom_hsiList = []
        for hsimg in hsiList:
            decom_hsiData = hsimg[:,:,numDecom//2:numDecom//2+n_components]
            decom_hsiList.append(decom_hsiData)
        
        return decom_hsiList
    
    else:
        raise ValueError('Unknown decompose method')
