...
def std_for_hsiList(hsi_dataset_list, average):
    ...
    return [np.sqrt(np.sum(np.square(hsi_list[:,i,:,:] - average[i])) / (length * shape[2] * shape[3])) for i in range(shape[1])]
